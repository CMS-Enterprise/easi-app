...
**MacOS:**
Install the version of Go that matches what's in our [Dockerfile](../Dockerfile) (currently go 1.26.8) with `brew install go@1.26`.

**Windows+WSL:**
- Download the `.tar.gz` file for the correct version of Go (matches what's in our [Dockerfile](../Dockerfile) - currently go 1.26.8) for Linux from [the official Go site](https://go.dev/doc/install), making sure to save it to the Ubuntu filesystem. The easiest way to do this is to copy the download link, then use `wget` to download it on the command line, i.e. `wget https://go.dev/dl/go1.26.8.linux-amd64.tar.gz`. This will download the `.tar.gz` to the current directory.
- From the directory containing the `.tar.gz` file, extract it to `/usr/local/go`, i.e. with `sudo tar -C /usr/local -xzf go1.26.8.linux-amd64.tar.gz`.
