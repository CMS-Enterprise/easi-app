...
**MacOS:**
Install the version of Go that matches what's in our [Dockerfile](../Dockerfile) (currently go 1.24.4) with `brew install go@1.24.1`.

**Windows+WSL:**
- Download the `.tar.gz` file for the correct version of Go (matches what's in our [Dockerfile](../Dockerfile) - currently go 1.24.4) for Linux from [the official Go site](https://golang.org/doc/install), making sure to save it to the Ubuntu filesystem. The easiest way to do this is to copy the download link, then use `wget` to download it on the command line, i.e. `wget https://go.dev/dl/go1.24.1.linux-amd64.tar.gz`. This will download the `.tar.gz` to the current directory.
