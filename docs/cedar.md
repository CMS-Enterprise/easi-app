...
EASi API calls are made directly to the CEDAR API. EASi previously ran an nginx caching proxy in front of CEDAR to expedite data-retrieval (see [ADR 0040](./adr/0040-CEDAR-caching.md)), but it was decommissioned in 2026 (EASI-5051) after being bypassed via a runtime flag for about a year. While it did improve response times, it also masked upstream CEDAR API issues.
