describe.only('GRB review', () => {
  ...
  it('Adds time to voting and Ends time to voting', () => {
    ...
    cy.getByTestId('date-picker-external-input').type('08/01/2028');
    ...
    cy.getByTestId('grb-review-async-end-date').should(
      'contain.text',
      `This review will now end on 08/01/2028.`
    );
    ...
    cy.getByTestId('alert').contains(
      `You added time to this GRB review. The new end date is 08/01/2028 at 5:00pm EST.`
    );

    cy.getByTestId('grb-review-status-card').should(
      'contain.text',
      `Review ends 08/01/2028, 5:00pm EST`
    );
  ...
  it('Restart GRB review', () => {
    ...
    cy.get('[role="dialog"]')
      ...
      .within(() => {
        ...
        cy.getByTestId('date-picker-external-input').type('07/01/2027');
    ...
    cy.getByTestId('alert').should(
      'contain.text',
      `You restarted this GRB review. The new end date is 07/01/2027 at 5:00pm EST.`
    );

    cy.getByTestId('grb-review-status-card').should(
      'contain.text',
      `Review ends 07/01/2027, 5:00pm EST`
    );
  });
});
