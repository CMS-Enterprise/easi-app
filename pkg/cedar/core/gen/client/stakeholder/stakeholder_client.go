...
// New creates a new stakeholder API client with basic auth credentials.
...
// New creates a new stakeholder API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) StakeholderFindList(params *StakeholderFindListParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*StakeholderFindListOK, error) {
	...
	if params == nil {
		params = NewStakeholderFindListParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "stakeholderFindList",
		Method:             "GET",
	...
	success, ok := result.(*StakeholderFindListOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for stakeholderFindList: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
