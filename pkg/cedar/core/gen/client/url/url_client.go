...
// New creates a new url API client with basic auth credentials.
...
// New creates a new url API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) URLFindList(params *URLFindListParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*URLFindListOK, error) {
	...
	op := &runtime.ClientOperation{
		...
		Params:             params,
		Reader:             &URLFindListReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*URLFindListOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for urlFindList: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
