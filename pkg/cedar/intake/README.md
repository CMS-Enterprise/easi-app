...
- `go run cmd/test_cedar_intake/main.go submit` will call our Intake client to send data to CEDAR. It requires that `CEDAR_INTAKE_DISABLED`, `CEDAR_API_URL`, and `CEDAR_API_KEY` environment variables to be set; these should be set in your .envrc.local, pointing to the CEDAR impl environment.
