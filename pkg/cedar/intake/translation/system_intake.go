...
func (si *TranslatableSystemIntake) CreateIntakeModel(ctx context.Context) (*wire.IntakeInput, error) {
	...
	obj := &intakemodels.EASIIntake{
		...
		CurrentEstimatedCost:                 si.CurrentEstimatedCost.Ptr(),
		CurrentEstimatedCostITPortion:        si.CurrentEstimatedCostITPortion.Ptr(),
		DecisionState:                        string(si.DecisionState),
		DigitalServiceInteraction:            pStringEnum(si.DigitalServiceInteraction),
		DigitalServiceInteractionDescription: si.DigitalServiceInteractionDescription.Ptr(),
		DoesNotSupportSystems:                si.DoesNotSupportSystems.Ptr(),
		DraftBusinessCaseState:               string(si.DraftBusinessCaseState),
		EstimatedTotalContractValue:          si.EstimatedTotalContractValue.Ptr(),
		EstimatedTotalContractValueITPortion: si.EstimatedTotalContractValueITPortion.Ptr(),
		FinalBusinessCaseState:               string(si.FinalBusinessCaseState),
		GovernanceTeamsIsPresent:             si.GovernanceTeamsIsPresent.Ptr(),
		LCIDType:                             pStringEnum(si.LCIDType),
		LCIDComponent:                        pStringEnum(si.LCIDComponent),
		LCIDIsLowIT:                          si.LCIDIsLowIT,
		LCIDIsShortened:                      si.LCIDIsShortened,
		LifecycleExpirationAlertTS:           si.LifecycleExpirationAlertTS,
		LifecycleRetiresAt:                   si.LifecycleRetiresAt,
		LifecycleIssuedAt:                    si.LifecycleIssuedAt,
		PriorityAlignment:                    si.PriorityAlignment.Ptr(),
		ProtectedCmsDataAccessedOutside:      pStringEnum(si.ProtectedCmsDataAccessedOutside),
		ProtectedCmsDataAccessedOutsideDescription: si.ProtectedCmsDataAccessedOutsideDescription.Ptr(),
		RequestFormState:          string(si.RequestFormState),
		State:                     string(si.State),
		Step:                      string(si.Step),
		TRBFollowUpRecommendation: pStringEnum(si.TRBFollowUpRecommendation),
