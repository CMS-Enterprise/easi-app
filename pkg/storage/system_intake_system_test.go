	...
	s.Run("sets systems on a system intake", func() {
		...
		for _, systemIntakeID := range createdIDs {
			err := sqlutils.WithTransaction(ctx, s.db, func(tx *sqlx.Tx) error {
				return s.store.SetSystemIntakeSystems(ctx, tx, systemIntakeID, systemNumbers, []*models.SystemRelationship{})
			})
			s.NoError(err)
		}
		...
		err = sqlutils.WithTransaction(ctx, s.db, func(tx *sqlx.Tx) error {
			return s.store.SetSystemIntakeSystems(ctx, tx, createdIDs[0], append(systemNumbers, system4), []*models.SystemRelationship{})
		})
		s.NoError(err)
