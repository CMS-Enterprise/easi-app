...
func (s *Store) CreateTRBGuidanceLetterInsight(
	...
	if err != nil {
		appcontext.ZLogger(ctx).Error(
			...
			zap.Error(err),
			zap.String("user", insight.CreatedBy),
		)
		return nil, err
	}
	defer stmt.Close()
...
// GetTRBGuidanceLetterInsightsSharingTRBRequestID queries the DB for all TRB guidance letter insights with the same TRB request ID as the given insight
// It will not return any entities that have a deleted_at value
