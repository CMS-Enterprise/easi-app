...
func GetTestPrincipal(store *storage.Store, userName string, UserSearchClient usersearch.Client, isAdmin bool) *authentication.EUAPrincipal {
	...
	userAccount, err := userhelpers.GetOrCreateUserAccount(context.Background(), store, userName, true, userhelpers.GetUserInfoAccountInfoWrapperFunc(UserSearchClient.FetchUserInfo))
	if err != nil {
		panic(err)
	}
