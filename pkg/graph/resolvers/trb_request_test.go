...
func (s *ResolverSuite) TestCreateTRBRequest() {
	...
	trb, err := CreateTRBRequest(s.testConfigs.Context, models.TRBTBrainstorm, s.fetchUserInfoStub, s.testConfigs.Store, nil)
	s.NoError(err)
	s.NotNil(trb)
...
func (s *ResolverSuite) TestGetTRBRequests() {
	...
	trb2, err := CreateTRBRequest(s.testConfigs.Context, models.TRBTBrainstorm, s.fetchUserInfoStub, s.testConfigs.Store, nil)
	s.NoError(err)
	s.NotNil(trb2)
	//Check for 2 request
	col, err = GetTRBRequests(s.testConfigs.Context, false, s.testConfigs.Store)
