...
func CreateSystemIntakeDocument(
	...
	uploaderRole, err := resolveUploaderRole(ctx, intake)
	if err != nil {
		return nil, err
	}
	...
	documentDatabaseRecord.CreatedBy = appcontext.Principal(ctx).Account().Username
...
func allowView(ctx context.Context, store *storage.Store, s3Key string) error {
	...
	grbUsers, err := store.SystemIntakeGRBReviewersBySystemIntakeIDs(ctx, []uuid.UUID{document.SystemIntakeID})
	if err != nil {
		return err
	}
	...
	appcontext.ZLogger(ctx).Warn("unauthorized user attempted to view system intake document",
		...
		zap.String("system_intake.id", document.SystemIntakeID.String()),
		zap.String("document.id", document.ID.String()))

	return errors.New("unauthorized attempt to view system intake document")
}

// resolveUploaderRole returns the uploader role for a new system intake document.
// Admins are always recorded as ADMIN, otherwise the acting user must match the intake requester.
func resolveUploaderRole(ctx context.Context, intake *models.SystemIntake) (models.DocumentUploaderRole, error) {
	...
	if err != nil {
		return "", fmt.Errorf("problem fetching system intake requester: %w", err)
	}

	if requester == nil {
		return "", errors.New("system intake requester not found")
	}
	...
	appcontext.ZLogger(ctx).Warn("unauthorized user attempted to upload system intake document",
		zap.String("username", user.Username),
		zap.String("system_intake.id", intake.ID.String()))

	return "", errors.New("user is not authorized to upload system intake documents")
}
