		...
		for _, tag := range uniqueTags {
			...
			case models.TagTypeGroupGrbReviewers:
				...
				for id := range grbReviewerCache {
					...
					// do not send an additional email to the initial poster as they will have already received one
					// letting them know that someone has replied
					if id == initialPoster.ID {
						continue
					}
