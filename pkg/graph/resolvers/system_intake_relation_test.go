...
func (suite *ResolverSuite) TestSetSystemIntakeRelationNewSystem() {
	...
	for caseName, caseValues := range cases {
		suite.Run(caseName, func() {
			...
			updatedIntakeContractNumbers, err := SystemIntakeContractNumbers(suite.ctxWithNewDataloaders(), openIntake.ID)
			suite.NoError(err)
			...
			updatedIntakeSystemIDs, err := SystemIntakeSystems(suite.ctxWithNewDataloaders(), openIntake.ID)
			suite.NoError(err)
			...
			updatedIntakeContractNumbers, err = SystemIntakeContractNumbers(suite.ctxWithNewDataloaders(), updatedIntake.ID)
			suite.NoError(err)
...
func (suite *ResolverSuite) TestUnlinkSystemIntakeRelation() {
	...
	suite.Run("unlink new system intake", func() {
		...
		// skip the following test, see Note [EASI-4160 Disable Contract Number Linking]
		// nums, err := SystemIntakeContractNumbers(suite.ctxWithNewDataloaders(), unlinkedIntake.ID)
		// suite.NoError(err)
		// suite.Empty(nums)

		// Check system IDs are cleared
		systemIDs, err := SystemIntakeSystems(suite.ctxWithNewDataloaders(), unlinkedIntake.ID)
		suite.NoError(err)
