...
func (suite *ResolverSuite) TestSetTRBRequestRelationNewSystem() {
	...
	for caseName, caseValues := range cases {
		suite.Run(caseName, func() {
			...
			updatedTRBRequestContractNumbers, err := TRBRequestContractNumbers(suite.ctxWithNewDataloaders(), trbRequest.ID)
			suite.NoError(err)
			...
			updatedTRBRequestSystemIDs, err := TRBRequestSystems(suite.ctxWithNewDataloaders(), trbRequest.ID)
			suite.NoError(err)
			...
			updatedTRBRequestContractNumbers, err = TRBRequestContractNumbers(suite.ctxWithNewDataloaders(), updatedTRBRequest.ID)
			suite.NoError(err)
...
func (suite *ResolverSuite) TestUnlinkTRBRequestRelation() {
	...
	suite.Run("unlink new trb request", func() {
		...
		nums, err := TRBRequestContractNumbers(suite.ctxWithNewDataloaders(), unlinkedTRBRequest.ID)
		suite.NoError(err)
		suite.Empty(nums)

		// Check system IDs are cleared
		systemIDs, err := TRBRequestSystems(suite.ctxWithNewDataloaders(), unlinkedTRBRequest.ID)
		suite.NoError(err)
