...
func (s ServicesTestSuite) TestNewCreateSystemIntakeAction() {
	...
	s.Run("returns QueryError if fetch fails", func() {
		...
		createAction := NewCreateSystemIntakeAction(failFetch, submit, review, review, review, review, review)
		id := uuid.New()
	...
	s.Run("returns error from an action service", func() {
		...
		createAction := NewCreateSystemIntakeAction(fetch, failSubmit, review, review, review, review, review)
		id := uuid.New()
		action := models.Action{
			IntakeID:   &id,
			ActionType: models.ActionTypeSUBMIT,
	...
	s.Run("returns ResourceConflictError if invalid action type", func() {
		createAction := NewCreateSystemIntakeAction(fetch, submit, review, review, review, review, review)
		id := uuid.New()
		action := models.Action{
			IntakeID:   &id,
			ActionType: "INVALID",
