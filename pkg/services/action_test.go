...
func (s ServicesTestSuite) TestNewTakeAction() {
	...
	s.Run("returns QueryError if fetch fails", func() {
		...
		createAction := NewTakeAction(failFetch, submit, review, review, review, review, review, review, review)
		id := uuid.New()
	...
	s.Run("returns error from an action service", func() {
		...
		createAction := NewTakeAction(fetch, failSubmit, review, review, review, review, review, review, review)
		id := uuid.New()
		action := models.Action{
			IntakeID:   &id,
			ActionType: models.ActionTypeSUBMIT,
	...
	s.Run("returns ResourceConflictError if invalid action type", func() {
		createAction := NewTakeAction(fetch, submit, review, review, review, review, review, review, review)
		id := uuid.New()
		action := models.Action{
			IntakeID:   &id,
			ActionType: "INVALID",
