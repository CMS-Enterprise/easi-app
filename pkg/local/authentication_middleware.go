...
func devUserContext(ctx context.Context, authHeader string, store *storage.Store) (context.Context, error) {
	...
	userAccount, err := userhelpers.GetOrCreateUserAccount(ctx, store.DB, store, princ.ID(), true, userhelpers.GetUserInfoAccountInfoWrapperFunc(localOktaClient.FetchUserInfo))
	if err != nil {
		return nil, err
	}
