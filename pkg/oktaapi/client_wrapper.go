...
func (cw *ClientWrapper) SearchCommonNameContains(ctx context.Context, searchTerm string) ([]*models.UserInfo, error) {
	...
	// Sanitize searchTerm for non-word characters
	specialCharsAndNumsRegex := regexp.MustCompile(`[^\w\s]`)
	searchTerm = specialCharsAndNumsRegex.ReplaceAllString(searchTerm, "")
